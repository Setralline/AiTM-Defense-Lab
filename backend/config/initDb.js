...
const createInitialAdmin = async () => {
  try {
    ...
    // [FIX] DETERMINISTIC PASSWORD LOGIC
    // If running in TEST environment, use fixed password to prevent test failures.
    // Otherwise, generate a random secure password for security.
    let adminPass;
    
    if (process.env.NODE_ENV === 'test') {
        adminPass = 'lab123';
        console.log('\x1b[33m%s\x1b[0m', ' [TEST MODE] Using fixed admin password: lab123');
    } else {
        // Generate a Strong Password that satisfies the Regex:
        // Min 8 chars, 1 Uppercase, 1 Lowercase, 1 Number/Symbol
        const randomPart = crypto.randomBytes(6).toString('hex'); // 12 chars (a-f, 0-9)
        adminPass = `${randomPart}A1!`; // Forces compliance
    }
